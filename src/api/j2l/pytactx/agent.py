...
def exploreSourcesDirs(rootpath, subdirs=[], takeOnlyExts=['.py', '.json'], dontTakeExts=[],
                       dontTakeFolders=["venv", "j2l"], takeHidden=False, recursive=True, explored={}):
    ...
    for filefullname in filenames:
        ...
        if (len(fileext) == 0):  # Recursive folder explore
            ...
            exploreSourcesDirs(rootpath, subdirsRecursive, takeOnlyExts, dontTakeExts, dontTakeFolders, takeHidden,
                               recursive, explored)
        else:
...
class Agent(IAgent):
    def __init__(self, playerId: str or None = None, arena: str or None = None, username: str or None = None,
                 ...
                 imgOutputPath: str or None = "img.jpeg", autoconnect: bool = True, waitArenaConnection: bool = True,
                 verbosity: int = 3, robotId: str or None = "_", welcomePrint: bool = True,
        ...
        if (welcomePrint):
            ...
            print("Hi there 👋")
            print(
                "To see the arena in your web browser 🎮, go to 👉 https://play.jusdeliens.com/?viewer=tactx&arena=" + str(
                    arena) + "&url=" + str(server) + "&usr=" + str(username) + "&pwd=" + str(
                    passwordParam) + "&pseudo=" + str(playerId))
        ...
        self.robot: rbx.IRobot = rbx.OvaClientMqtt(robotId, arena, username, password, server, port, imgOutputPath,
                                                   autoconnect, True, verbosity, playerId, False)
    ...
    def _onRobotConnected(self, eventSrc: Any, eventName: str, playerState: dict[str, Any]) -> None:
        ...
        self.robot.playMelody(
            [('E4', 30), ('G4', 30), ('C5', 30), ('G4', 30), ('C5', 30), ('E5', 30), ('C5', 30), ('E5', 30), ('G5', 30),
             ('C6', 30)])
    ...
    def _onPlayerChanged(self, eventSrc: Any, eventName: str, playerState: dict[str, Any]) -> None:
        for playerKey, playerValue in playerState.items():
            ...
            anx.debug(
                "♟️ Player attribute " + attributeName + " changed from " + str(attributeValueBefore) + " to " + str(
    ...
    def _onArenaChanged(self, eventSrc: Any, eventName: str, arenaState: dict[str, Any]) -> None:
        ...
        for gameKey, gameValue in arenaState.items():
            ...
            anx.debug("🎲 Game attribute " + attributeName + " changed from " + str(attributeValueBefore) + " to " + str(
                self.__dict__[attributeName]))
    ...
    def __run__(filepath: str = "settings.json", agentInfos=None):
        ...
        anx.setLogger(anx.FileLogger(
            ...
            os.path.join(agentInfos["logDir"],
                         agentInfos["arena"] + "_" + agentInfos["id"] + str(datetime.now()).replace(" ", "_").replace(
                             ":", "-") + ".log"))
        )
        ...
        anx.info("⏳ Agent " + str(agentInfos["playerId"]) + " starting on " + str(
            agentInfos["brokerAddress"]) + " with username " + str(agentInfos["username"]))
...
class AgentFr(IAgentFr):
    def __init__(self, nom: str or None = None, arene: str or None = None, username: str or None = None,
                 password: str or None = None, url: str or None = None, port: int = 1883,
                 fluxImage: str or None = "img.jpeg", autoconnect: bool = True, proxy: bool = True, verbosite: int = 3,
                 robotId: str or None = "_", welcomePrint=True, sourcesdir: str or None = None):
        ...
        if (welcomePrint):
            ...
            print("Hey 👋")
            print(
                "Pour visualiser l'arene de jeu 🎮, allez sur 👉 https://play.jusdeliens.com/?viewer=tactx&arena=" + str(
                    arene) + "&url=" + str(url) + "&usr=" + str(username) + "&pwd=" + str(
                    passwordParam) + "&pseudo=" + str(nom))
...
class AgentFrCibleAleatoire(AgentFr):
    def __init__(self, nom: str or None = None, arene: str or None = None, username: str or None = None,
        ...
        super().__init__(nom=nom, arene=arene, username=username, password=password, url=url, port=port,
                         fluxImage=fluxImage, autoconnect=autoconnect, proxy=proxy, verbosite=verbosite,
                         robotId=robotId,
